(function () {
  ...
  function isLoggedIn() {
    // Preferred: Liquid-injected truth if you already have it
    if (typeof window.__customerLoggedIn !== "undefined") return !!window.__customerLoggedIn;

    // Fallback: ShopifyAnalytics commonly has customerId when logged in
    try {
      var id = window.ShopifyAnalytics && ShopifyAnalytics.meta && ShopifyAnalytics.meta.page && ShopifyAnalytics.meta.page.customerId;
      return !!id;
    } catch (e) {
      return false;
    }
  ...
  function redirectToLogin() {
    // Return to the exact page (cart or saved cart) after login
